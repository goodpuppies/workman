...
schema-v2 slice through one static same-module fragment, direct `f32` numbers, one option-like ADT,
immutable control flow, and direct self-tail recursion. Automatic coercion, uniforms/resources,
`u32`/compute work, the broader promotion vocabulary, HM-shape specialization, and higher-order
closure work are post-v1. The visual product boundary is in `markdown/wmslang/v1-scope.md`; its
...
`slice_ir.wm` folds immutable blocks, retains restricted ADT matches, and validates direct self-tail
recursion as specified in `markdown/wmslang/v1-functional-lowering.md`. The next implementation
slice is ANF and minimal control-flow lowering. The closed v1 operator rows are specified in
