...
Why more than one field:
  ...
  span says where that rule frame lives in source
...
Why more than one field:
  ...
  frame span is the whole construct being checked
...
There are no nullable or optional fields in the core. Absence is represented by an empty collection
or by choosing a different tagged variant.
...
type SourceAnchor =
  ...
  | { kind: "imported"; module: ModuleId; exportedName: NameId; span?: Span }
...
`rule` names the compiler or language rule. `path` names the semantic logic branch that led there.
This is not a JavaScript stack trace. It is the compiler's rule path.
...
type Predicate =
  ...
  | { kind: "has-type"; subject: SubjectId; ty: TypeId }
...
The support graph is the state snapshot. It is a graph, not a fixed record of compiler subsystems.
...
support:
  failure entry: recursive binding result check failed
  premise entry: body result must equal recursive placeholder result
